...
- (void)setUp
{
    ...
    flag=[dataSource flagToSet:self];
	...
	shouldWrap = YES;
    scrollSpeed =30;//[delegate srollSpeed:self];
    bounceDistance = 1.0f;
    toggle = 0.0f;
...
#pragma setting parameters

-(void)numberOfItems:(int)aNumberOfItems
{
...
-(void)angleOfCarousel:(float)aAngle
{
...
-(void)itemWidth:(float)aItemWidth
{
...
- (void)setDataSource:(id<iCarouselDataSource>)aDataSource
{
...
- (void)setType:(iCarouselType)aType
{
...
- (void)setVertical:(BOOL)aVertical
{
...
- (void)setNumberOfVisibleItems:(NSInteger)aNumberOfVisibleItems
{
    if (numberOfVisibleItems != aNumberOfVisibleItems)
    {
...
- (void)setContentOffset:(CGSize)aContentOffset
{
   
    if (!CGSizeEqualToSize(contentOffset, aContentOffset))
    {
...
- (void)setViewpointOffset:(CGSize)aViewpointOffset
{
    if (!CGSizeEqualToSize(viewpointOffset,aViewpointOffset))
    {
        viewpointOffset = aViewpointOffset;
        [self layOutItemViews];
    }
}

- (void)setUseDisplayLinkIfAvailable:(BOOL)aUseDisplayLink
{
    if (useDisplayLink != aUseDisplayLink)
    {
        useDisplayLink = aUseDisplayLink;
        if (timer)
        {
            [self stopAnimation];
            [self startAnimation];
...
{	
    ...
    switch (iCarouselTypeWheel)
    {
        ...
        case iCarouselTypeInvertedWheel:
        {
            ...
            CGFloat arc = [self valueForTransformOption:iCarouselTranformOptionArc withDefault:M_PI * 2.0f];
            radius = [self valueForTransformOption:iCarouselTranformOptionRadius withDefault:itemWidth * (CGFloat)count / arc];
            //CGFloat angle =[delegate carouselAngle:self];
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    if (flag==1) 
    {
        ...
        view.center = CGPointMake(view.bounds.size.width/2.0f, view.bounds.size.height/2.0f);
        view.superview.center = CGPointMake(self.bounds.size.width/2,
                                            self.bounds.size.height/2.0f + contentOffset.height);
    }
    ...
    if (flag==2) 
    {
        ...
        view.center = CGPointMake(view.bounds.size.width/2.0f, view.bounds.size.height/2.0f);
        view.superview.center = CGPointMake(self.bounds.size.width,
                                            self.bounds.size.height/2.0f + contentOffset.height);
    }
    ...
    if (scrollEnabled)
    {
        ...
        switch (panGesture.state)
        {
            ...
            default:
            {
                ...
                scrollOffset=[panGesture translationInView:self].y/2+temp;
